...
const PROGRESS_FILE = path.join(
  __dirname,
  "../exercise-download-progress.json",
);

// CLI 옵션 파싱
const args = process.argv.slice(2);
const startIndex =
  parseInt(args.find((a) => a.startsWith("--start="))?.split("=")[1]) || 0;
const endIndex =
  parseInt(args.find((a) => a.startsWith("--end="))?.split("=")[1]) || Infinity;
...
const KOREAN_TO_ENGLISH_MAP = {
  ...
  // PNF 및 대각선 패턴 (ExerciseDB 검색 최적화)
  "상지 D1 굴곡 패턴": "cable one arm diagonal raise", // 유사 동작
  "상지 D1 신전 패턴": "cable crossover", // 유사 동작
  "상지 D2 굴곡 패턴": "sword draw", // 칼 뽑기 동작
  "상지 D2 신전 패턴": "cable diagonal raise",
  "PNF 찹 패턴 (양손)": "cable woodchop",
  "PNF 리프트 패턴 (양손)": "cable reverse woodchop",
  "네발기기 PNF 대각선 패턴": "bird dog",
  "상지 D1 굴곡 패턴 (밴드)": "band diagonal raise",
  "상지 D2 굴곡 패턴 (밴드)": "band sword draw",
  "하지 D1 굴곡 패턴": "cable hip flexion",
  "방산 유도 PNF 운동": "isometric",
  "리드믹 개시 PNF 패턴": "stretch",
};
...
function getEnglishName(koreanName) {
  ...
  for (const [korean, english] of Object.entries(KOREAN_TO_ENGLISH_MAP)) {
    ...
    if (
      normalizedClean.includes(koreanClean) ||
      koreanClean.includes(normalizedClean)
    ) {
      return english;
    }
  }
...
async function main() {
  ...
  console.log(
    `처리할 운동: ${targetTemplates.length}개 (인덱스 ${startIndex} ~ ${Math.min(endIndex, templates.length) - 1})\n`,
  );
  ...
  for (let i = 0; i < targetTemplates.length; i++) {
    ...
    if (englishFromParentheses) {
      ...
      console.log(
        `  📝 괄호에서 추출: "${englishFromParentheses}" -> "${searchQuery}"`,
      );
    }
    ...
    console.log(
      `  🔍 최종 검색어: "${searchQuery}" (원본: "${template.name}")`,
    );

    // 이미 파일이 있는지 확인
    const existingFile = path.join(
      OUTPUT_DIR,
      `${sanitizeFilename(template.name)}.gif`,
    );
    ...
    try {
      ...
      // API 응답이 배열이 아닐 수 있음
      let exerciseList = Array.isArray(results)
        ? results
        : results
          ? [results]
          : [];

...
function generateTemplates(): ExerciseTemplateInput[] {
  const templates: ExerciseTemplateInput[] = [];
  let id = 1;
...
function main() {
  ...
  const outputPath = join(process.cwd(), "templates", "exercise-templates-100.json");
  
  writeFileSync(outputPath, JSON.stringify(templates, null, 2), "utf-8");

...
다음 이미지는 REHAB 앱의 핵심 기능인 재활 코스 생성 플로우를 보여줍니다.

![재활 코스 생성 플로우](./images/rehab-course-generation-flow.png)
...
#### 왼쪽 화면: 헬스장 검색 및 부위 선택
- **헤더**: "REHAB" 앱 타이틀
- **검색 바**: "네 주변 재활 헬스장 찾기?" 검색 기능
- **지도 뷰**: 주변 헬스장 위치 표시
- **하단 카드**: 
  - 헬스장 정보 (이름, 이용 횟수, 시간)
  - 태그 시스템 (조용함, 재활 친화, 기구 구비 등)
  - 부위 선택 UI ("등/허리" 태그)
  - 재활 기구 선택 UI
...
- **입력 섹션**:
  - 부위 선택: "어느 부위가 불편한가요?"
  - 통증 정도: 1-5점 척도 (예시: 3점 선택)
  - 기구 선택: "오늘 가능한 기구 선택해주세요"
  - 총 운동 시간: 90분 선택
  - "코스 생성하기" 버튼
- **생성된 코스**:
  - **준비 운동 (10분)**: 가벼운 스트레칭
...
**참고 사항:**
- 이미지 파일은 `docs/images/rehab-course-generation-flow.png`에 저장되어 있습니다.
- 실제 구현 시 이 플로우를 참고하여 UI 컴포넌트를 개발합니다.

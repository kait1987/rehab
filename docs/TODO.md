...
##### 운동 데이터 보강
- [x] 운동 데이터 보강 (등 3→7개, 가슴 4→5개, 팔 5→6개, 다리 5→7개)
- [x] 맨몸 운동 추가: 코어 브레이싱(허리), 프론 수퍼맨/바닥 백 익스텐션(등)
- [x] 골반 맨몸 운동 추가: 골반 틸트, 서서 골반 회전, 스탠딩 힙 서클, 90-90 스트레칭, 레그 스윙
- [x] 매트 운동에 "없음" 매핑 추가 (브릿지, 버드독, 플랭크, 데드버그 등 7개)

##### 알고리즘 수정
- [x] 비활성 운동 필터링 수정 (`merge-body-parts.ts` L99-102 isActive 체크)
- [x] 기구 필터링 로직 개선 (`some()` → `every()` - 모든 필수 기구 보유 확인)
- [x] 메인 운동 intensity 수정 (목/손목/팔꿈치/등 부위 → intensity 3으로 업그레이드)
- [x] 중복 key 에러 수정 (운동 반복 시 고유 key 생성)

##### 검증 완료
- [x] 전체 부위별 운동 충분성 검증 (12개 부위 모두 통과)
- [x] 코스 생성 시나리오 테스트 (허리/등/골반/무릎/어깨+맨몸 모두 통과)
- [x] 데이터 무결성 검증 (intensity, 기구매핑, 부위매핑 완전성 확인)
...
- [x] 단위 테스트 작성 ✅ (95개 테스트, Vitest)
- [x] 다중 부위 병합 테스트 ✅ (deduplicate-exercises, calculate-priority, classify-by-section)
- [x] 금기운동 필터링 테스트 ✅ (filter-contraindications 23개 테스트)
- [x] 시간 조정 로직 테스트 ✅ (distribute-time 22개 테스트)

#### 6.2 API 기본 테스트 구축

- [x] 헬스장 검색 API 테스트 ✅ (gyms-search.test.ts - 24개)
- [x] 코스 생성 API 테스트 ✅ (rehab-generate.test.ts - 33개)
- [x] 리뷰 작성/조회 API 테스트 ✅ (reviews.test.ts - 36개, courses-save.test.ts - 31개)
- [x] 에러 케이스 테스트 ✅ (error-cases.test.ts - 52개)

> **총 176개 API 테스트 완료** | GitHub Actions CI/CD 통합

#### 6.3 E2E 테스트

- [x] 사용자 플로우 테스트 (온보딩 → 코스 생성 → 헬스장 찾기) ✅
- [x] 리뷰 작성 플로우 테스트 ✅
- [x] 예외 상황 테스트 (위치 권한 거부, 네트워크 끊김 등) ✅

> **Playwright 기반 ~37개 E2E 테스트 완료** | Page Object Model 적용

#### 6.4 클라우드 서버 배포

- [x] Vercel 배포 설정 ✅ (사용자 완료)
- [x] 환경변수 설정 ✅ (사용자 완료)
- [x] Supabase 프로젝트 설정 ✅ (사용자 완료)
- [x] 도메인 연결 (선택) ✅
- [x] HTTPS 설정 확인 ✅ (Vercel 자동 적용)

#### 6.5 모니터링 및 로깅

- [x] 에러 로깅 설정 (Sentry) ✅ (2026-01-11)
- [x] API 호출량 모니터링 ✅ (Vercel Analytics)
- [x] 사용자 이벤트 트래킹 (events 테이블) ✅
- [x] 성능 모니터링 ✅ (Sentry Performance)
...
- [x] 이용약관 페이지 작성 ✅ (`/terms` - 2026-01-11)
- [x] 개인정보 처리방침 페이지 작성 ✅ (`/privacy` - 2026-01-11)
- [x] 서비스 포지셔닝 명시 (의료행위 아님) ✅
- [x] 통증 악화 시 중단·전문의 상담 권고 문구 ✅

#### 7.2 코스 화면 안내 문구

- [x] 모든 코스 화면에 "의료행위 아님" 문구 표시 ✅ (Footer에 상시 표시)
- [x] 통증 악화 시 중단 안내 ✅
- [x] 전문의 상담 권고 문구 ✅

#### 7.3 보안 설정

- [x] HTTPS 적용 확인 ✅ (Vercel 자동)
- [x] 환경변수 보안 확인 ✅ (비밀키는 NEXT_PUBLIC_ 없음)
- [x] API 키 노출 방지 ✅ (SERVICE_ROLE_KEY 서버 전용)
- [x] XSS 방지 확인 ✅ (React 기본 이스케이핑)
...
- [x] 이용약관 및 개인정보 처리방침 페이지 제공 ✅ (2026-01-11)
...
- [x] 더 정교한 운동 추천 (추천 API 구현 완료)
- [x] 더 많은 부위/템플릿 확장 (관리자 CRUD 구현 완료)
- [x] 사용자 기록 관리 (히스토리 페이지 구현 완료)
- [x] 관리자 페이지 구축 (대시보드 및 관리 기능 구현 완료)

---

### 📌 1. 재활 운동 엔진 v2

- [x] 템플릿 200개로 확장 ✅ (202개 데이터 확보 및 시드 구현, 2026-01-12)
- [x] 부위 Bank 50개 이상 확장 ✅ (55개 계층 구조 데이터 및 시드 구현, 2026-01-12)
- [x] 운동 제외 로직(금기 조건) 고도화 ✅ (`contraindication-engine.ts` 구현, 2026-01-12)
- [x] 사용자 상태 기반 자동 난이도 조절 ✅ (`difficulty-engine.ts` 구현, 2026-01-12)
- [x] "오늘의 추천 코스" 기능 ✅ (`/api/courses/recommended` API 구현, 2026-01-12)

---

### 📌 2. 장소 데이터 고도화

- [x] 헬스장 데이터 사용자 제보 기능 ✅ (2026-01-12 구현 완료)
- [x] 운영시간 검증 로직 ✅ (2026-01-12 Lite: 수동 검증 플래그)
- [x] 리뷰 신뢰도 점수 시스템 ✅ (2026-01-12 Lite: 도움이 돼요 투표)
- [x] 헬스장 정렬 옵션(거리/영업중/리뷰순) ✅ (기존 구현 완료)

---

### 📌 3. 사용자 기능 강화

- [x] 지난 운동 기록 저장 ✅ (`/api/users/history` API 구현, 2026-01-12)
- [x] 주차별 운동 히스토리 UI ✅ (`/my/history` 페이지 구현, 2026-01-12)
- [x] 부상 부위 프로필화 ✅ (`/api/users/pain-profile` + `/my/profile` 구현, 2026-01-12)
- [x] 사용자 변화(통증, 가동범위) 저장 기능 ✅ (`/api/users/progress` + `/my/progress` 구현, 2026-01-12)

---

### 📌 4. 관리자/운영자 기능 ✅ (2026-01-12 구현 완료)

- [x] 관리자 인증 시스템 ✅ (Clerk `publicMetadata.role` 기반, `docs/ADMIN_SETUP.md`)
- [x] 관리자 미들웨어 가드 ✅ (`/admin/*` 경로 보호, `src/middleware.ts`)
- [x] 관리자 대시보드 ✅ (`/admin` 페이지, 4개 지표 카드)
- [x] 템플릿 CRUD 페이지 ✅ (`/admin/templates` 목록, `/admin/templates/[id]` 편집)
- [x] 리뷰 관리 페이지 ✅ (`/admin/reviews` 숨김/삭제/승인 액션)
- [x] 헬스장 정보 관리 페이지 ✅ (`/admin/gyms` 목록)
- [x] 권한 없음 페이지 ✅ (`/unauthorized`)

---

## 🟦 PHASE 2 완료 기준 ✅ (2026-01-12 완료)

- [x] 개인 기록 기반 맞춤 재활 시스템이 동작 ✅ (P2-F1-01 구현, 2026-01-12)
- [x] 리뷰·헬스장 정보 품질이 자동으로 개선 ✅ (P2-F2-01/02/03 구현, 2026-01-12)
- [x] 관리자 페이지를 통한 운영 가능 ✅ (2026-01-12)
...
### 📌 1. 웨어러블 연동 준비 ✅ (2026-01-13 완료)

- [x] HealthKit/Google Fit 구조 설계 ✅ (WearableData 모델 + API)
- [x] 걸음 수 기반 피로도 데이터 활용 로직 ✅ (calculate-fatigue.ts)
- [x] 심박 기반 운동 강도 조정 로직 설계 ✅ (calculate-intensity-from-hr.ts)
- [x] iOS/Android 연동은 미진행(사전 구조만 준비) ✅

 ...
 * @returns 검증 성공 여부
 */
export function validateAndLogDatabaseUrl(
  databaseUrl?: string
): boolean {
  ...
  if (!result.isValid) {
    ...
    if (url) {
      ...
      console.error(`DATABASE_URL 시작 부분: ${url.substring(0, 50)}...`);
      console.error(`DATABASE_URL 끝 부분: ...${url.substring(url.length - 30)}`);
      console.error(`DATABASE_URL에 @ 포함: ${url.includes('@') ? '✅' : '❌'}`);
      console.error(`DATABASE_URL에 :// 포함: ${url.includes('://') ? '✅' : '❌'}`);
      console.error(`포트 6543 (풀러) 포함: ${url.includes(':6543') ? '✅' : '❌'}`);
      console.error(`포트 5432 (직접) 포함: ${url.includes(':5432') ? '⚠️ 직접 연결' : '✅'}`);
      console.error(`pgbouncer 파라미터: ${url.includes('pgbouncer=true') ? '✅' : '❌'}`);
      ...
      try {
        ...
        console.error(`파싱된 사용자명: ${testUrl.username || '(없음)'}`);
      } catch (parseError) {
        console.error(`URL 파싱 실패: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
      }
    } else {
      console.error('\n[디버깅 정보]');
      console.error('DATABASE_URL 환경 변수가 존재하지 않습니다.');
      console.error('process.env.DATABASE_URL:', process.env.DATABASE_URL);
      console.error('process.env 키 목록:', Object.keys(process.env).filter(key => key.includes('DATABASE')));
    }

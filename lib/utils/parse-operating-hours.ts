...
export function parseOperatingHoursFromDescription(
  ...
  // 6. 전체 요일 동일하게 적용
  return createOperatingHoursForAllDays(
    processedTimePattern,
    breakTimeInfo.notes,
  );
}
...
function parseDaySpecificHoursWithPriority(
  ...
  const { specificDays, weekdayDays, weekendDays, allDays } =
    detectDayKeywords(description);
  ...
  for (const [day, timePattern] of specificDays.entries()) {
    if (!processedDays.has(day)) {
      const finalTimePattern =
        timePattern.openTime && timePattern.closeTime
          ? timePattern
          : defaultTimePattern;

 ...
 */
export const DIFFICULTY_SCORE_RANGES: Record<DifficultyLevel, { min: number; max: number }> = {
...
export const DIFFICULTY_DESCRIPTIONS: Record<DifficultyLevel, string> = {
  principle: '기초 단계 - 재활 운동의 기본 원리 이해, 낮은 강도, 안전한 동작 위주',
 ...
 * 경험 수준 → 기본 난이도 매핑
 */
export const EXPERIENCE_TO_DIFFICULTY: Record<ExperienceLevel, DifficultyLevel> = {
 ...
 * 통증 정도별 허용 난이도 범위
 */
export const PAIN_LEVEL_DIFFICULTY_RANGES: Record<number, { min: number; max: number }> = {
  5: { min: 1, max: 3 },  // 통증 5점: 원리만 허용

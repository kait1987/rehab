...
describe('distributeTime', () => {
  ...
  describe('60분 코스', () => {
    it('60분 코스의 시간 배분이 올바르다 (warmup ~10분, cooldown ~10분)', () => {
      ...
      const warmupTime = warmupExercises.reduce((sum, e) => sum + (e.durationMinutes || 0), 0);
      const cooldownTime = cooldownExercises.reduce((sum, e) => sum + (e.durationMinutes || 0), 0);
    ...
    it('60분 코스의 main 시간이 약 40분이다', () => {
      ...
      const mainTime = mainExercises.reduce((sum, e) => sum + (e.durationMinutes || 0), 0);
  ...
  describe('120분 코스', () => {
    ...
    it('120분 코스의 main 시간이 약 90분이다', () => {
      ...
      const exercises = {
        warmup: [EXERCISES.childPose],
        main: [EXERCISES.latPulldown, EXERCISES.pushUp, EXERCISES.lunge, EXERCISES.seatedRow],
        cooldown: [EXERCISES.catStretch],
      };
  ...
  describe('운동 시간 제한', () => {
    ...
    it('warmup/cooldown 운동당 최대 시간이 10분이다', () => {
      ...
      warmupCooldown.forEach(exercise => {
        expect(exercise.durationMinutes).toBeLessThanOrEqual(10);
      });
    });
  ...
  describe('복합 시나리오', () => {
    ...
    it('운동 수가 적어도 시간이 올바르게 배분된다', () => {
      ...
      // Assert
      const totalTime = result.reduce((sum, e) => sum + (e.durationMinutes || 0), 0);

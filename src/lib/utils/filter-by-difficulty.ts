...
export function filterByDifficulty(
  ...
  return exercises.filter((exercise) => {
    // difficultyScore 사용 (기본값 3) - null/undefined만 대체
    const difficultyScore = exercise.difficultyScore ?? 3;
    ...
    if (allowBoundary) {
      ...
      if (targetLevel === "principle") {
        ...
        return (
          (difficultyScore >= range.min - boundaryRange &&
            difficultyScore < range.min) ||
          (difficultyScore > range.max &&
            difficultyScore <= range.max + boundaryRange)
        );

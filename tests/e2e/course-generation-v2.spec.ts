...
test.describe("Engine v2 - Course Generation Flow", () => {
  ...
  test("코스 생성 기본 플로우가 정상 동작해야 함", async ({ page }) => {
    ...
    const hasStartButton = await page
      .getByRole("button", { name: /재활 코스 만들기/i })
      .isVisible();
    if (hasStartButton) {
      ...
      await expect(
        page
          .locator('[role="dialog"], [data-testid="onboarding-modal"]')
          .first(),
      )
...
test.describe("Engine v2 - Admin Templates Check", () => {
  test.skip("템플릿 목록에 200개 이상 존재해야 함", async ({ page }) => {
    // 1. 관리자 페이지 이동
    const response = await page.goto("/admin/templates");

    // 리다이렉트 발생 시 (로그인 페이지 등) 또는 권한 없음 상태 코드 스킵
    if (
    ...
    if (await totalText.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // 텍스트를 찾지 못함 -> 권한 문제일 가능성 높음
      // 명시적으로 실패하지 않고 스킵하거나 로그 남김
      console.log(
        "Admin template count text not found - possibly insufficient permissions",
      );
      test.skip();
    }
  });

...
test.describe.skip("F2: 코스 → 헬스장 찾기", () => {
  ...
  test.describe("위치 권한 허용", () => {
    ...
    test("현재 위치 기반 검색", async ({ page }) => {
      ...
      const locationBtn = page
        .locator(
          'button:has-text("현재 위치"), button:has-text("위치 찾기"), [data-testid*="location"]',
        )
        .first();
      ...
      const hasResults =
        (await page.locator('[data-testid^="gym-card"], .gym-card').count()) >
        0;
      const hasEmptyState =
        (await page
          .locator("text=검색 결과가 없습니다, text=주변에 헬스장이 없습니다")
          .count()) > 0;
    ...
    test("필터 적용 (재활기구)", async ({ page }) => {
      ...
      const filterBtn = page
        .locator('button:has-text("재활기구"), [data-testid*="filter"]')
        .first();
    ...
    test("헬스장 카드 클릭 → 상세 페이지 이동", async ({ page }) => {
      ...
      // 첫 번째 헬스장 카드 클릭
      const gymCard = page
        .locator('[data-testid^="gym-card"], .gym-card, article')
        .first();
  ...
  test.describe("위치 권한 거부", () => {
    ...
    test("위치 권한 거부 시 수동 입력 UI 표시", async ({ page }) => {
      ...
      const manualInput = page.locator(
        'input[placeholder*="주소"], input[placeholder*="위치"], [data-testid*="address"]',
      );
      const permissionMessage = page.locator(
        "text=위치 권한, text=위치를 허용, text=주소를 입력",
      );
    ...
    test("수동 주소 입력 검색", async ({ page }) => {
      ...
      const addressInput = page
        .locator('input[placeholder*="주소"], input[placeholder*="위치"]')
        .first();
      if (await addressInput.isVisible()) {
        ...
        const searchBtn = page
          .locator('button:has-text("검색"), button[type="submit"]')
          .first();
  ...
  test.describe("코스 → 헬스장 연결", () => {
    ...
    test("코스 결과에서 헬스장 찾기 버튼 표시", async ({ page }) => {
      ...
      // "근처 헬스장 찾기" 버튼 존재 확인
      const findGymBtn = page.locator(
        'button:has-text("헬스장"), a:has-text("헬스장")',
      );
      // 버튼이 있는지 확인 (코스 생성 후에만 표시될 수 있음)
      await expect(findGymBtn.first())

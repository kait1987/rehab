...
test.describe('예외: 네트워크 오류', () => {
  ...
  test.describe('헬스장 검색', () => {
    test('검색 API 실패 시 에러 메시지 표시', async ({ page }) => {
      // API Route 차단
      await page.route('**/api/gyms/search**', route => route.abort('failed'));
      
      const gymsPage = new GymsPage(page);
      await gymsPage.goto();
      
      // 검색 시도
      const searchBtn = page.locator('button:has-text("검색"), [data-testid*="search"]').first();
  ...
  test.describe('코스 생성', () => {
    ...
    test('코스 생성 API 실패 시 에러 메시지', async ({ page }) => {
      // API Route 차단
      await page.route('**/api/rehab/generate**', route => route.abort('failed'));
      ...
      const errorUI = page.locator('text=오류, text=실패, [data-testid="error"]');
      await expect(errorUI.first()).toBeVisible({ timeout: 10000 });
    });
    ...
    test('코스 생성 타임아웃 시 안내', async ({ page }) => {
      ...
      const timeoutMsg = page.locator('text=시간 초과, text=timeout, text=오류');
      await expect(timeoutMsg.first()).toBeVisible({ timeout: 35000 }).catch(() => {});
    });
  });

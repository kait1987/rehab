...
test.describe('예외: 검색 결과 없음', () => {
  ...
  test('검색 결과 없을 때 Empty State 표시', async ({ page }) => {
    // 빈 결과 반환 Mock
  ...
  test('Empty State에서 필터 초기화 버튼', async ({ page }) => {
    // 빈 결과 반환 Mock
    await page.route('**/api/gyms/search**', route => {
      route.fulfill({
        status: 200,
  ...
  test('Empty State에서 반경 확장 제안', async ({ page }) => {
    // 빈 결과 반환 Mock
    await page.route('**/api/gyms/search**', route => {
      route.fulfill({
        status: 200,
    ...
    if (await expandRadiusUI.first().isVisible({ timeout: 5000 }).catch(() => false)) {
      await expect(expandRadiusUI.first()).toBeVisible();
    }
  });

  test('코스 생성 시 적절한 운동 없음', async ({ page }) => {
    // 운동 없음 응답 Mock
    ...
    if (await generateBtn.isEnabled()) {
      ...
      await expect(guidance.first()).toBeVisible({ timeout: 10000 }).catch(() => {});
    }
  });
});
